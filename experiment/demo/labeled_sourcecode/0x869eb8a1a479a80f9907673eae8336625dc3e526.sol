...
contract CryptoDuel is Agent, SafeMath {
    ...
    function GetWin(uint _duelID) external {
        ...
        if (blockhash(duel.blocknumber) == 0 || (block.number - duel.blocknumber) > 256) {
            ...
            if (hash % 2 == 0) {
                duel.creator.transfer(sum); //storage tainted by rollback
                winner = duel.creator;
            ...
            if (RefAddr[winner] != address(0)) {                
                refFee = refLevel1Fee;
                rewardAddr[RefAddr[winner]] = safeAdd(rewardAddr[RefAddr[winner]], safePerc(duel_fee, refLevel1Fee)); //storage tainted by rollback
    ...
    function withdrawRefAddr() external returns (bool success) {
        ...
        msg.sender.transfer(amount); \\storage tainted by rollback (ASD dependency)
        emit withdrawProfit(amount, msg.sender);
        return true;
    }
    ...
    function withdrawRefBoth() external returns (bool success) {
        ...
        rewardAddr[msg.sender] = 0;
        rewardGroup[msg.sender] = 0; \\storage tainted by rollback (ASD)
        msg.sender.transfer(amount);
        emit withdrawProfit(amount, msg.sender);
        return true;
    }
