...
contract LotteryRound is LotteryRoundInterface, Owned {
    ...
    if (winners.length > 0) {
      for (uint i = 0; i < winners.length; i++) {
        ...
        if (unclaimed) {
          ...
          if (!winner.send(prizeValue)) {  //storage tainted by dos
            ...
            winningsClaimable[winner] = true; //storage tainted by dos
          }
        }
      }
    }
  ...
  function claimPrize() afterDraw {
    ...
    if (!msg.sender.send(prizeValue)) { // storage tainted by dos (ASD)
