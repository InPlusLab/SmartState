...
contract Bounty is WhitelistedRole, IBounty, Pausable {
  ...
  function openBounty(uint256 bountyId)
    ...
    for (uint8 i = 0; i < bounty.tokenAddress.length; i++) {
      require(IERC20(bounty.tokenAddress[i]).transfer(msg.sender, bounty.tokenAmount[i])); //storage tainted by dos
    }

    IERC721(bountyNFTAddress).burn(bountyId);
    delete bountyIdToBounty[bountyId]; //storage tainted by dos
  ...
  function checkBounty(uint256 bountyId) external view returns (
    ...
    address[],
    uint256[]) {
    Bounty storage bounty = bountyIdToBounty[bountyId]; //storage tainted by dos (TSD)
