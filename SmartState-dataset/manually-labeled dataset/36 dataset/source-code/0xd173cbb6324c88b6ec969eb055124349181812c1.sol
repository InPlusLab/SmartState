    ...
    function getRefund() public {
        ...
        for (uint i = 0; i < totalTickets; i++) {
            if (msg.sender == contestants[i].addr && raffleId == contestants[i].raffleId) {
                refunds++;
                contestants[i] = Contestant(address(0), 0, 0); \\storage tainted by dos (TSD)
                gaps.push(i);
        ...
        if (msg.sender == rakeAddress) {
            ...
            for (uint i = 0; i < totalTickets; i++) {
                if (raffleId == contestants[i].raffleId) {
                    ...
                    contestants[i].addr.transfer(pricePerTicket); \\ storage tainted by dos
                }
            }

            RaffleResult(raffleId, totalTickets + 1, address(0));
            raffleId++; \\ storage tainted by dos
            nextTicket = 0;
            gaps.length = 0;
        }
    }
