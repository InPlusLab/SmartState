    ...
    function pay() internal {
        ...
        for (uint i = 0; i < queue.length; i++){
            ...
            if (money >= leftPayout) { //If we have enough money on the contract to fully pay to investor
                ...
                if (leftPayout > 0) {
                    dep.depositor.send(leftPayout); // Send money to him  //storage tainted by dos
                    money -= leftPayout;
                }

                // this investor is fully paid, so remove him
                depositNumber[dep.depositor] = 0; //storage tainted by dos
                delete queue[idx];
