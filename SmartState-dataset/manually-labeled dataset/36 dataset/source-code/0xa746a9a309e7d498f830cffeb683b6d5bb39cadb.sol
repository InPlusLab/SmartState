...
contract CashPokerProICO is Ownable, Pausable {
    ...
    function finalSendTokens() public onlyOwner {
        ...
        for (uint i = sendInvestorIndex; i < holdTokenInvestors.length; i++) {
            ...
            if (tokenAmount > 0) {
                holdTokens[investor] = 0; // storage tainted by dos
                require(token.transferFrom(tokenWallet, investor, tokenAmount)); //storage tainted by dos 
            }

            if (msg.gas < 100000) {
                sendInvestorIndex = i; //storage tainted by dos 
                return;
            }
        }
