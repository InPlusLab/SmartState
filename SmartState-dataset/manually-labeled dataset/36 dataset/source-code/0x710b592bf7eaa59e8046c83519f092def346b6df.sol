  ...
  */
  ...
  function bet(uint8 number, uint8 betType) payable public {
    ...
    */
    ...
    bets.push(Bet({                 \\storage tainted by rollback (cross-method)
      betType: betType,
      player: msg.sender,
      number: number
    }));
  ...
  function spinWheel() public {
    ...
    for (uint i = 0; i < bets.length; i++) {
      ...
      if (won) {
        winnings[b.player] += betAmount * payouts[b.betType]; \\storage tainted by rollback
      }
    }
    /* delete all bets */
    bets.length = 0;
  ...
  function cashOut() public {
    ...
    winnings[player] = 0;
    player.transfer(amount); \\storage tainted by rollback
  }
