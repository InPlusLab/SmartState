...
contract Greedy is Owned {
    ...
    function buyHeart(address referred) public payable {
        ...
        if (msg.value >= 100000000000000000){
            ...
            if (luckyBuy() == true) 
            {
                msg.sender.transfer(Luckybuy); \\storage tainted by rollback
                emit luckybuyEvent(msg.sender, Luckybuy, Round);
                luckybuyTracker_ = 0; 
                Luckybuy = 0; \\storage tainted by rollback
            }
        }
    ...
    function win() public {
        ...
        //Round End 
        RoundLastGreedyMan[Round].transfer(RoundETH[Round]); //storage tainted by rollback
