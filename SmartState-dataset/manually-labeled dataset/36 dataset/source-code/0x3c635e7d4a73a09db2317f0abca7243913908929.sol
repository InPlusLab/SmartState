...
*/
  ...
  function selectWinner() private {
    ...
    winner.transfer(amount); \\storage tainted by dos (cross-method)
    playerPool.length = 0;
    owner.transfer(this.balance); \\storage tainted by dos (cross-method)
...
*/
  ...
  function refund() public onlyBy(owner) payable {
    ...
    for (uint i = 0; i < playerPool.length; i++) {
      playerPool[i].transfer(100 finney); \\storage tainted by dos
    }
      playerPool.length = 0; \\storage tainted by dos
  }
